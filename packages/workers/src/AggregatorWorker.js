...
const FAST_WINDOWS = ['5m', '1h'];
const SLOW_WINDOWS = ['6h', '24h'];
...
class AggregatorWorker {
    ...
    async run(includeSlow = false) {
        ...
        try {
            const windows = includeSlow
                ...
                : [...FAST_WINDOWS];
            await Promise.all(windows.map((w) => this.aggregateWindow(w)));
    ...
    async aggregateWindow(window) {
        const interval = shared_1.WINDOW_TO_INTERVAL[window];
    ...
    `);
        ...
        rows.forEach((row, i) => {
            ...
            const score = row.tx_count * shared_1.TRENDING_WEIGHTS.txns_1h * 100 +
                normVolume * shared_1.TRENDING_WEIGHTS.volume_1h +
                row.unique_wallets * shared_1.TRENDING_WEIGHTS.new_wallets * 100;
            const base = i * 7;
            placeholders.push(`($${base + 1}, $${base + 2}, $${base + 3}, $${base + 4}, $${base + 5}, $${base + 6}, $${base + 7})`);
            values.push(row.pool_address, window, score, row.volume_usd, row.tx_count, priceChange, row.unique_wallets);
        });
        if (placeholders.length > 0) {
            await database_1.db.query(`INSERT INTO trending_scores
         ...
         ON CONFLICT (pool_address, win) DO UPDATE SET
           ...
           calculated_at = NOW()`, values);
        }
        // 写入 Redis
        const pipeline = database_1.redis.pipeline();
        for (const row of rows) {
            ...
            const score = row.tx_count * shared_1.TRENDING_WEIGHTS.txns_1h * 100 +
                normVolume * shared_1.TRENDING_WEIGHTS.volume_1h +
                row.unique_wallets * shared_1.TRENDING_WEIGHTS.new_wallets * 100;
            pipeline.hset(database_1.RedisKeys.aggregated(row.pool_address, window), 'volume_usd', row.volume_usd, 'tx_count', row.tx_count, 'price_change', priceChange, 'new_wallets', row.unique_wallets, 'score', score);
            pipeline.expire(database_1.RedisKeys.aggregated(row.pool_address, window), 90);
            // 更新排行 ZSET
            pipeline.zadd(database_1.RedisKeys.pairsRanking(window), score, row.pool_address);
        }
        await pipeline.exec();
    }
    // ─── Flush aggregated data back to pools table ───────────────
    async updatePoolsTable() {
    ...
    async buildRankings() {
        ...
        for (const window of shared_1.WINDOWS) {
            const topPools = await (0, database_1.query)(`
        SELECT pool_address, score FROM trending_scores
        WHERE win = $1
        ORDER BY score DESC
        LIMIT 500
      `, [window]);
            await database_1.redis.set(`ranking_list:${window}`, JSON.stringify(topPools.map((r) => r.pool_address)), 'EX', 60);
        }
        // 新上线 pairs（24h内）
        const newPairs = await (0, database_1.query)(`SELECT address FROM pools WHERE created_at > NOW() - INTERVAL '24 hours' ORDER BY created_at DESC LIMIT 100`);
        await database_1.redis.set('ranking_list:new', JSON.stringify(newPairs.map((r) => r.address)), 'EX', 60);
        // 涨幅榜
        const gainers = await (0, database_1.query)(`SELECT pool_address FROM trending_scores
       WHERE win = '24h' AND price_change > 0
       ORDER BY price_change DESC LIMIT 100`);
        await database_1.redis.set('ranking_list:gainers', JSON.stringify(gainers.map((r) => r.pool_address)), 'EX', 60);
    }
}
