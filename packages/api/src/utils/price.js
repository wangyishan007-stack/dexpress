...
exports.aerodromeSwapToPrice = aerodromeSwapToPrice;
exports.routeToUsd = routeToUsd;
exports.calcAmountUsd = calcAmountUsd;
 ...
 */
function aerodromeSwapToPrice(amount0In, amount1In, amount0Out, amount1Out, decimals0, decimals1) {
    // 找出哪个 token 是输入
 ...
 */
function routeToUsd(token0Addr, token1Addr, priceToken0InToken1, // token0 以 token1 计价的价格
ethUsdPrice) {
    const t0 = token0Addr.toLowerCase();
    const t1 = token1Addr.toLowerCase();
    let token0Usd = 0;
    let token1Usd = 0;
    if (shared_1.STABLECOINS.has(t1)) {
        // token1 是稳定币
        token0Usd = priceToken0InToken1;
        token1Usd = 1;
    }
    else if (t1 === shared_1.WETH_LOWER) {
        // token1 是 WETH
        token0Usd = priceToken0InToken1 * ethUsdPrice;
        token1Usd = ethUsdPrice;
    }
    else if (shared_1.STABLECOINS.has(t0)) {
        // token0 是稳定币
        token0Usd = 1;
        token1Usd = priceToken0InToken1 > 0 ? 1 / priceToken0InToken1 : 0;
    }
    else if (t0 === shared_1.WETH_LOWER) {
        // token0 是 WETH
 ...
 */
function calcAmountUsd(amount0, amount1, token0Usd, token1Usd) {
    if (token0Usd > 0)
        return Math.abs(amount0) * token0Usd;
    if (token1Usd > 0)
        return Math.abs(amount1) * token1Usd;
    return 0;
}
// ─── Chainlink ETH/USD ────────────────────────────────────────
let cachedEthPrice = 0;
let lastPriceFetch = 0;
async function getEthUsdPrice(client, forceRefresh = false) {
    const now = Date.now();
    // 缓存 60 秒
    ...
    try {
        const result = await client.readContract({
            address: shared_1.ADDRESSES.CHAINLINK_ETH_USD,
            abi: shared_1.CHAINLINK_ABI,
