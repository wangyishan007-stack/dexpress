  ...
  return (
    <div className="mb-3 md:mb-4">
      <div className="flex items-center justify-between border-b border-border pb-0">
        ...
        <button
          onClick={onManage}
          className="hidden md:flex items-center gap-2 text-sub hover:text-text transition-colors pb-2.5 flex-shrink-0 ml-4"
        >
          <IconManage />
...
function ShareWatchlistModal({ listId, onClose }: { listId: string; onClose: () => void }) {
  ...
  const shareUrl = `${typeof window !== 'undefined' ? window.location.origin : ''}/watchlist?shared=${listId}`
