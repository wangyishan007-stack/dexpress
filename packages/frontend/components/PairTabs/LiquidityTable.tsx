...
export function LiquidityTable({ lpHolders, subgraphData }: Props) {
  ...
  if (useSubgraph) {
    ...
    return (
      <div className="overflow-y-auto" style={{ maxHeight: 400 }}>
        <div className="grid grid-cols-[32px_1fr_60px_80px] md:grid-cols-[40px_1fr_70px_200px_60px_40px] gap-x-2 md:gap-x-3 px-3 md:px-5 py-2 text-[14px] text-header border-b border-border sticky top-0 bg-surface z-10">
          <span>#</span>
          <span>Address</span>
          <span className="text-right">%</span>
          <span className="hidden md:block text-center">Value</span>
          <span className="hidden md:block text-center">Pos</span>
        ...
        {providers.map((lp, i) => (
          <div
            ...
            className="grid grid-cols-[32px_1fr_60px_80px] md:grid-cols-[40px_1fr_70px_200px_60px_40px] gap-x-2 md:gap-x-3 px-3 md:px-5 py-2 text-[14px] border-b border-muted hover:bg-border/20 transition-colors"
          >
            ...
            {/* Mobile: plain value */}
            <div className="md:hidden tabular text-right text-text font-mono">{fmtUsd(lp.value_usd)}</div>
            {/* Desktop: value + progress bar */}
            ...
            <span className="hidden md:block tabular text-center text-sub">{lp.position_count}</span>
            <a
              href={`https://basescan.org/address/${lp.owner_address}`}
              target="_blank"
              rel="noopener"
  ...
  return (
    <div className="overflow-y-auto" style={{ maxHeight: 400 }}>
      <div className="grid grid-cols-[32px_1fr_60px_80px] md:grid-cols-[40px_1fr_70px_200px_40px] gap-x-2 md:gap-x-3 px-3 md:px-5 py-2 text-[14px] text-header border-b border-border sticky top-0 bg-surface z-10">
        <span>#</span>
        <span>Address</span>
        <span className="text-right">%</span>
        <span className="hidden md:block text-center">Value</span>
      ...
      {lpHolders!.map((lp, i) => {
        ...
        return (
          <div
            key={lp.address}
            className="grid grid-cols-[32px_1fr_60px_80px] md:grid-cols-[40px_1fr_70px_200px_40px] gap-x-2 md:gap-x-3 px-3 md:px-5 py-2 text-[14px] border-b border-muted hover:bg-border/20 transition-colors"
          >
            <span className="text-sub tabular">{i + 1}</span>
            <div className="flex items-center gap-1.5 min-w-0">
              <a
            ...
            <div className="md:hidden tabular text-right text-text font-mono">{fmtUsd(value)}</div>
            <div className="hidden md:flex items-center gap-2 min-w-0">
              ...
              <span className="text-sub tabular text-[13px] flex-shrink-0">{fmtUsd(totalValue)}</span>
            </div>
            <a
              href={`https://basescan.org/address/${lp.address}`}
              target="_blank"
